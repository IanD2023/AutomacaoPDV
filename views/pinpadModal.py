from envs import *
from scripts import verificarPINPAD as pinpad
...
class modalPinpad():
                ...
                def Main(self):
                    ...
                    opcaoerro = Label(self.container, text="Selecione o erro:",bg="#f2f2f2")
                    opcaoerro.place(relx=0.5, rely=0.10, relwidth=0.5, relheight=0.08, anchor=CENTER)           

                    opcao1 = Label(self.container, text="1 - Cartão mal inserido | \nNão está lendo nenhum cartão",bg="#f2f2f2")
                    opcao1.place(relx=0.5, rely=0.20, relwidth=0.5, relheight=0.08, anchor=CENTER)           

                    opcao5 = Label(self.container, text="2 - Erro de comunicação",bg="#f2f2f2")
                    opcao5.place(relx=0.5, rely=0.30, relwidth=0.5, relheight=0.08, anchor=CENTER)

                    labelinput = Label(self.container, text="Digite a opção e tecle ENTER:",bg="#f2f2f2")
                    labelinput.place(relx=0.5, rely=0.50, relwidth=0.5, relheight=0.08, anchor=CENTER)

                    inputOpcao=Entry(self.container, width=22, insertwidth= 2, justify="center",textvariable="1")
                    inputOpcao.focus()
                    inputOpcao.place(relx = 0.5, rely = 0.57, anchor = CENTER)
                    inputOpcao.bind('<Return>',lambda e:self.trocaUSB(botaoCancelar,opcao1,opcao5,[labelinput,opcaoerro],inputOpcao,msg_retorno))    
                    ...
                    msg_retorno.place(relx=0.5, rely=0.70, anchor=CENTER) 
                ...
                def resultado(self,Error):
                    ...
                    def result():
                        ...
                        if validarInput() == True:
                            ...
                            pinpad.verificarPinPad(Error,self.container,self,[opcao1,opcao5,labelinput,inputOpcao])
                ...
                def trocaUSB(self,botaoCancelar,opcao1,opcao5,widgts,input,Error):
                    ...
                    def validarInput():
                        ...
                        try:
                            ...
                            int(input.get())
                            if int(input.get()) > 2 or int(input.get()) <= 0: 
                                Error['text']="Opção inválida"
                                return False
                    ...
                    if validarInput() == True:
                        ...
                        if input['textvariable']=='1':
                            ...
                            opcao1['text']="Já trocou de porta USB?"
                            opcao1.place(relx=0.5, rely=0.17, relwidth=0.5, relheight=0.08, anchor=CENTER) 

                            opcao5['text']="1 - SIM"
                            
                            input['textvariable']='2'

                            return False

                        if input['textvariable'] == '2':
                            ...
                            pinpad.verificarPinPad(Error,self.container,self,[])
                ...
                def resultadoFinal(self):
                    ...
                    # novaTela=TelaPrincipal.topLevel()
                    self.telaPrincipal.withdraw() 
                    ...
                    def result(widgets):
                        ...
                        if validarInput() == True:
                            ...
                            for x in widgets: x.destroy()   
                            ##modal.children['!label']['font']=("Arial",10)
                            #novaTela.destroy() 
                            ...
                            return modalPinpad.resultadoFinal(self)                                               
                            #pinpad.verificarPinPad(Error,self.container,self,[])
